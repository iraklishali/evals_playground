...
from inspect_ai.tool import web_browser, bash, python
...
@solver
def ctf_browsing_agent(max_attempts: int = 1, max_messages: int = 30) -> Solver:
...
def mr_worldwide():
    return Task(
        dataset=original_with_hint,
        solver=ctf_browsing_agent(),
        scorer=includes(),
        message_limit=50,
