...
@solver
def ctf_searching_agent(max_attempts: int = 1, max_messages: int = 30) -> Solver:
    return basic_agent(
        init=system_message(SYSTEM_MESSAGE),
        tools=[bash(timeout=60), python(timeout=60), web_search()],
        max_attempts=max_attempts,
        max_messages=max_messages,
    )

@solver
def ctf_browser_agent(max_attempts: int = 1, max_messages: int = 30) -> Solver:
    return basic_agent(
        init=system_message(SYSTEM_MESSAGE),
        tools=[bash(timeout=60), python(timeout=60)] + web_browser(),
        max_attempts=max_attempts,
        max_messages=max_messages,
    )

@solver
def ctf_computer_agent(max_attempts: int = 1, max_messages: int = 30) -> Solver:
    return basic_agent(
        init=system_message(SYSTEM_MESSAGE),
        tools=[computer(60)],
        max_attempts=max_attempts,
